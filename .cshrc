...
if (! $?EC_ENV_ROOT) then
  if ( -f /usr/intel/pkgs/eclogin/1.0/etc/csh.cshrc ) then
    source /usr/intel/pkgs/eclogin/1.0/etc/csh.cshrc
  else if ( -f /usr/intel/common/pkgs/eclogin/1.0/etc/csh.cshrc ) then
...
if ($?prompt) then
  if ($?EC_ENV_ROOT && ! $?EC_ENV_LOGIN) then
    if ( -f $EC_ENV_ROOT/etc/csh.login ) then
      setenv EC_ENV_LOGIN 1
      source $EC_ENV_ROOT/etc/csh.login
    endif
  endif
endif
